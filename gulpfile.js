...
var metaData = {
    ...
    "APIURL" : apiurl,
    "APPKEY" : appkey,
    "VIEWSURL" : protocol+'://view.'+domain+port,
    "SITEPORT" : (protocol=="http"?8888:8889),
    "HTTP": protocol+"://",
    "SITES": '{"www.'+domain+port+'":"www","global.'+domain+port+'":"global","m.'+domain+port+'":"m","view.'+domain+port+'":""}',
    "VIEWSSITE": 'view.'+domain+port,
    "MSITE" : protocol+"://m."+domain+port,
    "SSITE" : protocol+"://sm."+domain+port,
    "CURURL":"",//当前路径的url
    "MODURL":""//当前模块的路径
}
...
gulp.task("track",function(){
    return gulp.src([config.VIEWS + "/m/track/**/*.html"])
        .pipe(fileinclude({ basepath: '@file' }))
        .pipe(gulp.dest(config._VIEWS + '/m/track/'))
});
...
gulp.task('build', function (callback) { runSequence('tsc', 'nodets', 'sass', 'files',"track", ['js','html','php','nodejs','buildViews','buildControllers','buildBundles'],"webpack", "includeFiles",callback) });
